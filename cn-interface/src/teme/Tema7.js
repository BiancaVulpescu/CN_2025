...
const Tema7 = () => {
  ...
  const handleRun = async () => {
    ...
    setOutput('');
    try {
      const res = await fetch('http://localhost:5000/tema7-output', {
        method: 'POST',
  ...
  return (
    <div>
      <h2>Metoda lui Halley de aproximare a radacinilor unui polinom (Tema 7)</h2>
      <button onClick={handleRun} disabled={loading} style={{marginBottom: '1em'}}>
        {loading ? 'Se execută...' : 'Rulează Tema 7'}
      </button>
